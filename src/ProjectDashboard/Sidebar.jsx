...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    { icon: <FaTasks size={18} />, label: 'Tasks' },
    { icon: <FaThLarge size={18} />, label: 'Board' },
    { icon: <FaUserPlus size={18} />, label: 'Add Members' },
    { icon: <FaUsers size={18} />, label: 'Team' },
    { icon: <FaTicketAlt size={18} />, label: 'Tickets' },
  ];
        ...
        <ul className="px-2 py-4 space-y-2">
          {navItems.map((item, index) => (
            ...
            >
              ...
              <div className="flex items-center justify-center w-6 h-6 flex-shrink-0 text-[#222831] hover:text-[#00ADB5] transition-colors duration-300">
                {item.icon}
              </div>
              ...
              <AnimatePresence mode="wait">
                {expanded && (
                  ...
                  >
                    <span className="text-sm font-medium text-[#222831] whitespace-nowrap block">
                      {item.label}
                    </span>
                  </motion.div>
                )}
